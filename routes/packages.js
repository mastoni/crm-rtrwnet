...
router.post('/', async (req, res) => {
    const { name, speed_down, speed_up, price, description, category, is_active } = req.body;
    try {
        ...
        await db.query(
            'INSERT INTO packages (id, name, speed_down, speed_up, price, description, category, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [newId, name, speed_down || 0, speed_up || 0, price || 0, description || '', category || 'home', is_active === undefined ? true : is_active]
        );
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (Object.keys(req.body).length === 1 && req.body.is_active !== undefined) {
            ...
            await db.query(
                'UPDATE packages SET name=?, speed_down=?, speed_up=?, price=?, description=?, category=?, is_active=? WHERE id=?',
                [name, speed_down, speed_up, price, description, category, is_active, req.params.id]
            );
        }
